...
def main():
    ...
    if st.session_state.col1.button("Process", key="process_btn"):
        if uploaded_pdf is not None:
            st.session_state.pdf_doc = uploaded_pdf
    ...
    # Handle question and render PDF
    if submitted and user_question:
        if st.session_state.conversation is not None:
            handle_userinput(user_question)

            # 🧠 Extract only nearby pages for display
            with NamedTemporaryFile(suffix="pdf") as temp:
                temp.write(st.session_state.pdf_doc.getvalue())
                temp.seek(0)
                ...
                with NamedTemporaryFile(suffix="pdf") as temp2:
                    pdf_writer.write(temp2)
                    temp2.seek(0)
                    base64_pdf = base64.b64encode(temp2.read()).decode("utf-8")
